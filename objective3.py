...
def orthogonal_regression(points):
    ...
    return slope,np.real(intercept)
...
def dist(point,centroids):
    distances=((centroids-point)**2).sum(1)
...
cluster_centroids= np.array([list(i) for i in zip(np.linspace(min(xs),max(xs),cc),np.linspace(max(ys),min(ys),cc))])
...
for a in range(iterations):
    clusters = [[] for _ in range(cc)]
    x = 0; y = 0
    for i in points:
        temp = dist(i,cluster_centroids)
    ...
    for ind, i in enumerate(clusters): 
        if i:
            xs,ys=zip(*i)
            l=len(i)
            cluster_centroids[ind]=[sum(xs)/l,sum(ys)/l]
...
for i in clusters:
    ...
    if temp.size !=0:
        ...
        lines_set.append((slope,intercept))
    else:
        lines_set.append((None,None))
        #sorry we didn't implement when some of our k clusters are empty, we didn't have the time
